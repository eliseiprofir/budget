...
def yearly_analytics():
    ...
    chart.altair_chart(positive_categories_chart, use_container_width=True)
    table.dataframe(positive_categories_df, use_container_width=True)
    
    monthly_positive_table = build_category_table(monthly_data, "positive_categories")
    st.dataframe(monthly_positive_table, use_container_width=True)
    
    monthly_positive_chart = build_category_chart(monthly_data, "positive")
    positive_chart = alt.Chart(monthly_positive_chart.reset_index()).mark_bar().encode(
        x=alt.X("Month:N", title=None, sort=list(calendar.month_name)[1:], axis=alt.Axis(labelAngle=0)),
        y=alt.Y("Total:Q", title=None),
    ...
    chart.altair_chart(negative_categories_chart, use_container_width=True)
    table.dataframe(negative_categories_df, use_container_width=True)
    
    monthly_negative_df = build_category_table(monthly_data, "negative_categories")
    st.dataframe(monthly_negative_df, use_container_width=True)
    ...
    chart.altair_chart(neutral_categories_chart, use_container_width=True)
    table.dataframe(neutral_categories_df, use_container_width=True)
    monthly_neutral_df = build_category_table(monthly_data, "neutral_categories")
    st.dataframe(monthly_neutral_df, use_container_width=True)
